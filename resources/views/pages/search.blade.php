...
<div id="content">
   <ol class="breadcrumb">
      ...
      <li class="active">tìm kiếm <span style="font-style: italic; font-weight: bold;">{{$keyword}}</span></li>
   </ol>
   @if(count($products) != 0)
   ...
   @else
   <div class="text-center h3" style="padding-top: 12%">Không tìm thấy kết quả</div>
   @endif
      ...
      $(window).scroll(function() {
         if($(window).scrollTop() + $(window).height() == $(document).height()) {
            if(currentPage != total){
               currentPage++;
               var request = $.ajax({
                  url: "http://localhost:8080/final/tim-kiem/"+currentPage+"?keyword={{$keyword}}",
                  method: "GET",
                  dataType: "html"
                });
